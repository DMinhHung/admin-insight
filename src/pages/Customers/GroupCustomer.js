...
const GroupCustomer = () => {
  ...
  const fetchGroupCustomers = async (nameSearch = '') => {
    ...
    try {
      const res = await api.get(
        `api/v1/admin/customer/group${nameSearch ? `?name=${nameSearch}` : ''}`
      );
  ...
  const handleEdit = async (record) => {
    try {
      setLoading(true);
      const res = await api.get(`/api/v1/admin/customer/group/view`, {
  ...
  const handleModalOk = async () => {
    try {
      ...
      setLoading(true);
      const url = editingId
        ? '/api/v1/admin/customer/group/update'
        : '/api/v1/admin/customer/group/create';
  ...
  const handleDelete = async (id) => {
    confirm({
      ...
      onOk: async () => {
        try {
          setLoading(true);
          const res = await api.post(`/api/v1/admin/customer/group/delete`, {
            id,
          });
  ...
  return (
    <div style={{ padding: 24 }}>
      <Row justify="space-between" align="middle" style={{ marginBottom: 24 }}>
        <Col>
          <Title level={2}>Nhóm Khách Hàng</Title>
        </Col>
        <Col>
          <Space>
            <Input
